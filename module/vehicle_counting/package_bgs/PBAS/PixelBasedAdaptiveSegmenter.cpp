...
{
	strcpy(configPath, "../../..");
	std::cout << "Congfig File Path " << configPath << std::endl;
...
void PixelBasedAdaptiveSegmenter::saveConfig()
{
	...
	sprintf(configFilePath, "%s/config/PixelBasedAdaptiveSegmenter.xml", configPath);
	CvFileStorage* fs = cvOpenFileStorage(configFilePath, 0, CV_STORAGE_WRITE);
...
void PixelBasedAdaptiveSegmenter::loadConfig()
{
	char configFilePath[_MAX_PATH];
	sprintf(configFilePath, "%s/config/PixelBasedAdaptiveSegmenter.xml", configPath);
	CvFileStorage* fs = cvOpenFileStorage(configFilePath, 0, CV_STORAGE_READ);
...
void PixelBasedAdaptiveSegmenter::setConfigPath(const char* cf)
{
	strcpy(configPath, cf);
	std::cout << "Set Config Path To " << configPath << std::endl;
}