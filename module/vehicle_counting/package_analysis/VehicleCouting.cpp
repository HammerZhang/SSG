	...
	void VehicleCouting_on_mouse(int evt, int x, int y, int flag, void* param)
	{
		...
		if (evt == CV_EVENT_LBUTTONDOWN)
		{
			if (!startDraw)
			{
				...
				roi_y0 = y;
				startDraw = 1;
				std::cout << "Down " << roi_defined<< std::endl;
			}
			else
			{
				roi_x1 = x;
...
void VehicleCouting::saveConfig()
{
	...
	sprintf(configFilePath, "%s/config/VehicleCouting.xml", configPath);
...
void VehicleCouting::setConfigPath(const char* cf)
{
	strcpy(configPath, cf);
	std::cout << "Reset Config Path to " << configPath << std::endl;
}